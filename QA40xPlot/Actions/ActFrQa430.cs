...
namespace QA40xPlot.Actions
{
	...
	public partial class ActFrQa430 : ActBase
	{
		...
		public ActFrQa430(MyViewClass vm) : base(vm)
		{
			...
			MyGuiModel.ShowMiniPlots = false;
			UpdateGraph(true);
		}
		...
		public async Task RunMeasurement(bool runContinuously)
		{
			...
			guiVm.HasExport = (PageData.GainFrequencies.Length > 0);
		}

		public void NextToData(DataTab page)
		{
		...
		public async Task RunAcquisition(AcquireStep qaCfg, int index)
		{
            ...
            DataTab NextPage = new DataTab(guiVm, lrts); // copy the settings into our new page
			...
			try
			{
				...
				foreach (var myConfig in variables)
				{
					...
					var u = lastCfg;
					lastCfg = await OpampViewModel.ExecuteModel(myConfig, lastCfg, guiVm.HasQA430); // update the qa430 if needed
					if(u != lastCfg)
					{
					...
					if (!CanToken.IsCancellationRequested)
					{
						...
						if(ttype == TestingType.InputZ)
						{
							...
							NextPage.GainData = ([], []); // new list of complex data
							NextPage.GainFrequencies = [];
                            QA430Model? model = guiVm.HasQA430 ? Qa430Usb.Singleton?.QAModel : null;
		...
		public override Rect GetDataBounds()
		{
			...
			if (guiVm.ShowLeft && msdre != null)
			{
				tabs.Add(new LeftRightResult
				{
					Left = msdre,
					Right = (guiVm.ShowRight || !is2Channel) ? msdim : [],
					XValues = msdfrq
				});
			}
		...
		private async Task WaitForSynch(DataTab page, TestingType ttype, double dfreq, double genVolt)
		{
			double delta = 2;
			double lastu = 1;
			int icnt = 0;
		...
		/// <returns></returns>
		private async Task<bool> RunFreqTest(DataTab page, double[] stepBinFrequencies, double voltagedBV)
		{
			var vm = (MyViewClass)page.ViewModel;
			await showProgress(0, 50);
		...
		/// <param name="voltagedBV">the sine generator voltage</param>
		/// <returns></returns>
		private async Task<bool> RunNoiseTest(DataTab page, AcquireStep rsStep, int index)
		{
			var vm = (MyViewClass)page.ViewModel;
			await showProgress(0, 50);
			// Check if cancel button pressed
		...
		private async Task<(LeftRightSeries?, Complex[], Complex[])> RunChirpAcquire(DataTab page, double voltagedBV, WaveChannels channels)
		{
			var vm = (MyViewClass)page.ViewModel;
		...
		private async Task WaitForChirpSync(DataTab page, double voltagedBV, WaveChannels channels)
		{
			double delta = 2;
			double lastu = 1;
			int icnt = 0;
		...
		public async Task<(LeftRightFrequencySeries?, List<Complex[]>, List<Complex[]>)> DoChirpTest(DataTab page, double voltagedBV, TestingType ttype, WaveChannels channels, int index)
		{
			var vm = (MyViewClass)page.ViewModel;
		...
		/// <param name="voltagedBV">the sine generator voltage</param>
		/// <returns></returns>
		private async Task<bool> RunChirpTest(DataTab page, double voltagedBV, int index)
		{
			var vm = (MyViewClass)page.ViewModel;
			var perOctave = (vm.SmoothingVal <= 0) ? 0 : vm.SmoothingVal;       // octave smoothing by default
			if (perOctave > 0.5)
